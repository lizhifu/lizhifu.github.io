...
设有一连串字符串abcdefedcbsabdabdkJBDVAI,字符串范围为0-9，A-Z,a-z。其中0-z为连续，其中最长连续回文子串为bcdefedcb，即最长连续回文子串长度为9。其中，在ascill表中9-A不连续，Z-a不连续，因此不能直接用ascill码表来计算连续。  
...
首先,题目要求比较多，有三个关键词： **最长子串** 、 **回文** 、 **连续** 。  
因此，第一步先不做那么难，先只求最长回文子串。  
